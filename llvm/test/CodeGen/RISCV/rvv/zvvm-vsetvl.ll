...
; The vtype constant is 0xC2 (bits: vma=1, vta=1, vlmul=0, vsew=2), small
; enough that constant materialization is a single addi.
...
; CHECK:       ret
  %vl = call iXLen @llvm.riscv.vsetvl.matrix.iXLen(
    ...
    iXLen 1,    ; VTA=1
    iXLen 1,    ; VMA=1
    iXLen 0,    ; LAMBDA=0 (use vtype.lambda)
    iXLen 0,    ; BS=0
    iXLen 0,    ; ALTFMT_A=0
    iXLen 0)   ; ALTFMT_B=0
  ret iXLen %vl
...
; -----------------------------------------------------------------------------
; Lambda=L4 (encoding 3). On RV32 lambda lives in bits [30:28], so the vtype
; constant has bit 29 set: 0x300000C2. RV64 places lambda 32 bits further up.
